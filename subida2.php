...
//Opción a modificar para controlar el tamaño total de las subidas(en este caso para 10 ficheros)
$tamanototal=0;
for ($x=0; $x < 10; $x++) {
...
//Si se superan los 100 MB, se redirige al usuario indicándole que ha sobrepasado el límite establecido
if ($tamanototal>=100*MB) {
	header('location:./subida.php?tamanototal=max');
	exit();
}//Si no hay ningún fichero, se le indicará al usuario que debe indicar al menos un fichero para subir:
if ($tamanototal==0) {
	header ("Location: subida.php?noarchivo=si ");
	exit();
}

//si el array $_FILES["archivo"] contiene un nombre, ósea,  se va a subir algún fichero:
$correcto=0;
for ($x=0; $x < 10; $x++) { 
	if(!empty($_FILES["archivo".$x]["name"])) {
		...
		//Por cada fichero subido correcto o erróneamente, se indicará con un mensaje si se ha subido o no acompañado del nombre de cada fichero:
